	...
	with legacy_playlists as (
		...
		where
			...
			or exists (
				...
				left join tube.modules tm
					on tm.slug = format('legacy-playlist-%s', pl.id)
				left join tube.videos tv
					on tv.youtube_video_id = lv.youtube_id
				left join tube.module_videos tmv
					on tmv.module_id = tm.id
					and tmv.video_id = tv.id
				where pv.playlist_id = pl.id
					and (
						tm.id is null
						or tv.id is null
						or tmv.module_id is null
					)
			)
		order by pl.created_at asc nulls last
		limit v_playlist_limit
	), priority_videos as (
		select distinct on (pv.youtube_id)
		...
		join public.playlist_videos pvv
			on pvv.video_id = pv.id
		join legacy_playlists lp
			on lp.legacy_playlist_id = pvv.playlist_id
		where not exists (
			select 1
			from tube.videos tv
			where tv.youtube_video_id = pv.youtube_id
		)
		order by pv.youtube_id, pv.created_at asc
	), backlog_videos as (
		select distinct on (pv.youtube_id)
			pv.id as legacy_video_id,
			pv.youtube_id,
		...
		and not exists (
			select 1
			from priority_videos prv
			where prv.youtube_id = pv.youtube_id
		)
		order by pv.youtube_id, pv.created_at asc
	), legacy_videos as (
		select
		...
		from (
			...
			select
				1 as priority,
				bkv.*
			from backlog_videos bkv
		) src
		order by src.priority asc, src.created_at asc
		limit v_video_limit
	), legacy_channels as (
		select
