...
create table if not exists tube.backfill_runs (
	...
	constraint tube_backfill_runs_status_check check (
		status = any (array['running', 'completed', 'failed'])
	)
);

create index if not exists idx_tube_backfill_runs_job_status_started
	on tube.backfill_runs (job_type, status, started_at desc);
create index if not exists idx_tube_backfill_runs_created_by
	on tube.backfill_runs (created_by);

drop trigger if exists trg_tube_backfill_runs_updated_at on tube.backfill_runs;
create trigger trg_tube_backfill_runs_updated_at
before update on tube.backfill_runs
for each row execute function tube.set_updated_at();

alter table tube.backfill_runs enable row level security;

drop policy if exists tube_editor_manage_backfill_runs on tube.backfill_runs;
create policy tube_editor_manage_backfill_runs
on tube.backfill_runs
for all
to authenticated
...
begin
	...
	with legacy_videos as (
		...
		select
			...
			tm.id as module_id
		from legacy_playlists lp
		join tube.modules tm
			on tm.slug = lp.module_slug
...
revoke all on function tube.backfill_legacy_to_tube(integer, integer, uuid, uuid) from public;
grant execute on function tube.backfill_legacy_to_tube(integer, integer, uuid, uuid) to service_role;
