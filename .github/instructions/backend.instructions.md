...
- Do not run `supabase config push --yes`. Push with `pnpx supabase config push --project-ref wvkjainfwsyiyfcmbtid`, inspect every CLI prompt, and accept only the intended config diff.
